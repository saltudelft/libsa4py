...
class Pipeline:
    ...
    def __init__(
        ...
        projects_path,
        output_dir,
        nlp_transf: bool = True,
        use_cache: bool = True,
        use_pyre: bool = False,
        use_tc: bool = False,
        dups_files_path=None,
        split_files_path=None,
    ):
        ...
        if dups_files_path is not None:
            clusters_rand_files = [
                l.pop(random.randrange(len(l))) for l in load_jsonl_gz(dups_files_path)
            ]
            self.duplicate_files = [
                f for l in load_jsonl_gz(dups_files_path) for f in l
            ]
            self.duplicate_files = set(self.duplicate_files).difference(
                set(clusters_rand_files)
            )
            self.is_file_duplicate = (
                lambda x: True if x in self.duplicate_files else False
            )
        else:
        ...
        self.split_dataset_files = (
            {f: s for s, f in csv.reader(open(split_files_path, "r"))}
            if split_files_path is not None
            else {}
        )
        ...
        self.logger = self.__setup_pipeline_logger(
            join(self.err_log_dir, "pipeline_errors.log")
        )
    ...
    def get_project_filename(self, project) -> str:
        ...
        """
        return join(
            self.processed_projects, f"{project['author']}{project['repo']}.json"
        )
    ...
    def apply_nlp_transf(self, extracted_module: dict):
        ...
        def fn_nlp_transf(fn_d: dict, nlp_prep: NLPreprocessor):
            ...
            fn_d["params"] = {
                nlp_prep.process_identifier(p): t for p, t in fn_d["params"].items()
            }
            fn_d["ret_exprs"] = [
                nlp_prep.process_identifier(r.replace("return ", ""))
                for r in fn_d["ret_exprs"]
            ]
            fn_d["params_occur"] = {
                p: [nlp_prep.process_sentence(j) for i in o for j in i]
                for p, o in fn_d["params_occur"].items()
            }
            fn_d["variables"] = {
                nlp_prep.process_identifier(v): t for v, t in fn_d["variables"].items()
            }
            fn_d["fn_var_occur"] = {
                v: [nlp_prep.process_sentence(j) for i in o for j in i]
                for v, o in fn_d["fn_var_occur"].items()
            }
            fn_d["params_descr"] = {
                nlp_prep.process_identifier(p): nlp_prep.process_sentence(
                    fn_d["params_descr"][p]
                )
                for p in fn_d["params_descr"].keys()
            }
            fn_d["docstring"]["func"] = nlp_prep.process_sentence(
                fn_d["docstring"]["func"]
            )
            fn_d["docstring"]["ret"] = nlp_prep.process_sentence(
                fn_d["docstring"]["ret"]
            )
            fn_d["docstring"]["long_descr"] = nlp_prep.process_sentence(
                fn_d["docstring"]["long_descr"]
            )
            return fn_d

        extracted_module["variables"] = {
            self.nlp_prep.process_identifier(v): t
            for v, t in extracted_module["variables"].items()
        }
        extracted_module["mod_var_occur"] = {
            v: [self.nlp_prep.process_sentence(j) for i in o for j in i]
            for v, o in extracted_module["mod_var_occur"].items()
        }

        for c in extracted_module["classes"]:
            c["variables"] = {
                self.nlp_prep.process_identifier(v): t
                for v, t in c["variables"].items()
            }
            c["cls_var_occur"] = {
                v: [self.nlp_prep.process_sentence(j) for i in o for j in i]
                for v, o in c["cls_var_occur"].items()
            }
            c["funcs"] = [fn_nlp_transf(f, self.nlp_prep) for f in c["funcs"]]

        extracted_module["funcs"] = [
            fn_nlp_transf(f, self.nlp_prep) for f in extracted_module["funcs"]
        ]
    ...
    def process_project(self, i, project):
        ...
        project_analyzed_files: dict = {
            project_id: {"src_files": {}, "type_annot_cove": 0.0}
        }
        try:
            ...
            project_files = [
                (
                    f,
                    f_r,
                    self.split_dataset_files[f_r]
                    if f_r in self.split_dataset_files
                    else None,
                )
            ...
            if len(project_files) != 0:
                    ...
                    try:
                        ...
                        project_analyzed_files[project_id]["src_files"][f_relative] = (
                            self.apply_nlp_transf(
                                Extractor()
                                .extract(read_file(filename), pyre_data_file)
                                .to_dict()
                            )
                            if self.nlp_transf
                            else Extractor.extract(
                                read_file(filename), pyre_data_file
                            ).to_dict()
                        )
                        ...
                        extracted_avl_types = project_analyzed_files[project_id][
                            "src_files"
                        ][f_relative]["imports"] + [
                            c["name"]
                            for c in project_analyzed_files[project_id]["src_files"][
                                f_relative
                            ]["classes"]
                        ]
                ...
                if len(project_analyzed_files[project_id]["src_files"].keys()) != 0:
                    project_analyzed_files[project_id]["type_annot_cove"] = round(
                        sum(
                            [
                                project_analyzed_files[project_id]["src_files"][s][
                                    "type_annot_cove"
                                ]
                                for s in project_analyzed_files[project_id][
                                    "src_files"
                                ].keys()
                            ]
                        )
                        / len(project_analyzed_files[project_id]["src_files"].keys()),
                        2,
                    )

                    save_json(
                        self.get_project_filename(project), project_analyzed_files
                    )
    ...
    def run(self, repos_list: List[Dict], jobs, start=0):
        ...
        repos_list = [
            p
            for p in repos_list
            if not (os.path.exists(self.get_project_filename(p)) and self.use_cache)
        ]
        print(
            f"Number of projects to be processed after considering cache: {len(repos_list)}"
        )

        start_t = time.time()
        ParallelExecutor(n_jobs=jobs)(total=len(repos_list))(
            delayed(self.process_project)(i, project)
            for i, project in enumerate(repos_list, start=start)
        )
        print(
            "Finished processing %d projects in %s "
            % (len(repos_list), str(timedelta(seconds=time.time() - start_t)))
        )
