    ...
    def leave_Expr(
        ...
        if match.matches(original_node, match.Expr(value=match.SimpleString())):
            return updated_node.with_changes(
                value=cst.SimpleString(value='"""[docstring]"""')
            )
        else:
            return updated_node
...
class StringRemover(cst.CSTTransformer):
    ...
    def leave_SimpleString(
        ...
        return (
            updated_node.with_changes(value='"[string]"')
            if updated_node.value != '"""[docstring]"""'
            else updated_node
        )
...
class TypeAdder(cst.CSTTransformer):
    ...
    def leave_FunctionDef(
        ...
        ret_type = self.module_type_annot[
            (
                self.cls_stack[-1] if len(self.cls_stack) > 0 else None,
                self.fn_stack[-1],
                None,
            )
        ][0]
    ...
    def leave_Name(self, original_node: cst.Name, updated_node: cst.Name):
        ...
        if len(self.cls_stack) > 0:
            if (
                ...
                original_node.value,
            ) in self.module_type_annot:  # skips classes' identifiers
                if (
                    self.module_type_annot[
                        (
                            self.cls_stack[-1],
                            self.fn_stack[-1] if len(self.fn_stack) > 0 else None,
                            original_node.value,
        ...
        return (
            updated_node.with_changes(value=f"${name_type}$")
...
class SpaceAdder(cst.CSTTransformer):
    ...
    def leave_Asynchronous(self, node, updated_node):
        return updated_node.with_changes(whitespace_after=cst.SimpleWhitespace(" "))
    ...
    def leave_ConcatenatedString(self, node, updated_node):
        return updated_node.with_changes(whitespace_between=cst.SimpleWhitespace(" "))
    ...
    def leave_RightCurlyBrace(self, node, updated_node):
        return updated_node.with_changes(whitespace_before=cst.SimpleWhitespace(" "))
    ...
    def leave_Del(self, node, updated_node):
        return updated_node.with_changes(whitespace_after_del=cst.SimpleWhitespace(" "))
...
class TypeQualifierResolver(cst.CSTTransformer):
    ...
    def leave_Annotation(
        ...
        if self.type_annot_visited:
            ...
            if self.parametric_type_annot_visited:
                ...
                if q_name is not None:
                    return updated_node.with_changes(
                        annotation=self.__name2annotation(q_name).annotation
                    )
    ...
    def leave_SubscriptElement(
        ...
        if self.type_annot_visited and self.parametric_type_annot_visited:
            if match.matches(
                original_node,
                match.SubscriptElement(slice=match.Index(value=match.Subscript())),
            ):
                ...
                original_node,
                match.SubscriptElement(slice=match.Index(value=match.Ellipsis())),
            ):
                # TODO: Should the original node be returned?!
                return updated_node.with_changes(slice=cst.Index(value=cst.Ellipsis()))
            elif match.matches(
                original_node,
                match.SubscriptElement(
                    slice=match.Index(value=match.SimpleString(value=match.DoNotCare()))
                ),
            ):
                return updated_node.with_changes(
                    slice=cst.Index(value=updated_node.slice.value)
                )
            elif match.matches(
                original_node,
                match.SubscriptElement(
                    slice=match.Index(value=match.Name(value="None"))
                ),
            ):
                return original_node
            elif match.matches(
                original_node,
                match.SubscriptElement(slice=match.Index(value=match.List())),
            ):
                return updated_node.with_changes(
                    slice=cst.Index(value=updated_node.slice.value)
                )
                ...
                if q_name is not None:
                    return updated_node.with_changes(
                        slice=cst.Index(value=self.__name2annotation(q_name).annotation)
                    )
    ...
    def __name2annotation(self, type_name: str):
        ...
        try:
            return match.extract(
                ...
                match.AnnAssign(
                    target=match.Name(value=match.DoNotCare()),
                    annotation=match.SaveMatchedNode(match.DoNotCare(), "type"),
                ),
            )["type"]
            ...
            if (
                ...
                return match.extract(
                    cst.parse_module("x: %s=None" % self.last_visited_name.value)
                    .body[0]
                    .body[0],
                    match.AnnAssign(
                        target=match.Name(value=match.DoNotCare()),
...
class ParametricTypeDepthReducer(cst.CSTTransformer):
    ...
    def leave_SubscriptElement(self, original_node, updated_node):
        if (
            self.max_annot > self.max_annot_depth
            and self.current_annot_depth == self.max_annot_depth
        ):
            self.max_annot -= 1
            return updated_node.with_changes(
                slice=cst.Index(value=cst.Name(value="Any"))
            )
        else:
            return updated_node
...
class TypeApplier(cst.CSTTransformer):
    ...
    def __get_fn(self, f_node: cst.FunctionDef) -> dict:
        ...
        for fn in fns:
            if fn["q_name"] == self.__get_qualified_name(f_node.name) and set(
                list(fn["params"].keys())
            ) == set(self.__get_fn_params(f_node.params)):
                return fn
    ...
    def __get_fn_params(self, fn_params: cst.Parameters):
        ...
        stararg = (
            ...
            if match.matches(
                fn_params.star_arg,
                match.Param(name=match.Name(value=match.DoNotCare())),
            )
            else []
        )
        for p in (
            list(fn_params.params)
            + list(fn_params.kwonly_params)
    ...
    def __get_var_type_assign_t(self, var_name: str):
        ...
        if len(self.cls_visited) != 0:
            if len(self.fn_visited) != 0:
                ...
                # A class variable
                if (
                    self.cls_visited[-1][1][var_name]
                    == self.last_visited_assign_t_count
                ):
    ...
    def __get_var_names_counter(self, node, scope):
        ...
        return Counter(
            [
                ...
                for n in vars_name
                if isinstance(
                    self.get_metadata(cst.metadata.ScopeProvider, n["name"]), scope
                )
            ]
        )

    def visit_ClassDef(self, node: cst.ClassDef):
        self.cls_visited.append(
            (
                self.__get_cls(node),
    ...
    def leave_SimpleStatementLine(
        ...
        if match.matches(
            ...
            original_node,
            match.SimpleStatementLine(
                body=[match.AnnAssign(target=match.Name(value=match.DoNotCare()))]
            ),
        ):
    ...
    def visit_AssignTarget(self, node: cst.AssignTarget):
        if match.matches(
            node, match.AssignTarget(target=match.Name(value=match.DoNotCare()))
        ):
    ...
    def leave_Module(self, original_node: cst.Module, updated_node: cst.Module):
        return updated_node.with_changes(
            body=self.__get_required_imports() + list(updated_node.body)
        )
    ...
    def __get_required_imports(self):
        def find_required_modules(all_types):
            ...
            for _, a_node in all_types:
                m = match.findall(
                    a_node.annotation,
                    match.Attribute(value=match.DoNotCare(), attr=match.DoNotCare()),
                )
                if len(m) != 0:
                    for i in m:
                        req_mod.add(
        ...
        all_type_names = set(
            chain.from_iterable(
                map(lambda t: regex.findall(r"\w+", t[0]), self.all_applied_types)
            )
        )
                ...
                cst.SimpleStatementLine(
                    body=[
                        cst.ImportFrom(
                            ...
                            names=[
                                cst.ImportAlias(name=cst.Name(value=t), asname=None)
                    ...
                    cst.SimpleStatementLine(
                        body=[
                            cst.Import(
                                names=[
                                    cst.ImportAlias(
                                        name=cst.Name(value=mod_name), asname=None
                                    )
                                ]
                            )
                        ]
...
class TypeApplierExtended(cst.CSTTransformer):
    ...
    def __get_var_type_assign_t(self, var_name: str, var_node):
        ...
        if len(self.cls_visited) != 0:
            if len(self.fn_visited) != 0:
                # A class method's variable
                if (
                    self.fn_visited[-1][0]["fn_var_ln"][var_name][0][0]
                    == var_line_no[0]
                ):
                    t = self.__get_fn_vars(self.nlp_p(var_name))
            else:
                # A class variable
                if (
                    self.cls_visited[-1][0]["cls_var_ln"][var_name][0][0]
                    == var_line_no[0]
                ):
                    t = self.__get_cls_vars(self.nlp_p(var_name))
        elif len(self.fn_visited) != 0:
    ...
    def __get_var_names_counter(self, node, scope):
        ...
        attr_name = match.extractall(
            node,
            match.OneOf(
                match.AssignTarget(
                    target=match.SaveMatchedNode(
                        match.Attribute(
                            value=match.Name(value=match.DoNotCare()),
        ...
        return Counter(
            ...
            + [
                ...
                for n in attr_name
                if isinstance(
                    self.get_metadata(cst.metadata.ScopeProvider, n["attr"]), scope
                )
            ]
        )
    ...
    def leave_SimpleStatementLine(
        ...
        if match.matches(
            ...
            match.SimpleStatementLine(
                body=[
                    match.Assign(targets=[match.AssignTarget(target=match.DoNotCare())])
                ]
            ),
        ):
            if match.matches(
                ...
                t = self.__get_var_type_assign_t(
                    original_node.body[0].targets[0].target.attr.value,
                    original_node.body[0].targets[0].target,
                )
        ...
        # Typed variables
            ...
            if match.matches(
                ...
                t = self.__get_var_type_an_assign(
                    original_node.body[0].target.attr.value
                )
            if t:
                t_annot_node_resolved = self.resolve_type_alias(t)
                t_annot_node = self.__name2annotation(t_annot_node_resolved)
                    ...
                    body=[
                        cst.Assign(
                            targets=[
                                cst.AssignTarget(
                                    target=original_node.body[0].target,
                                    whitespace_before_equal=original_node.body[
                                        0
                                    ].equal.whitespace_before,
                                    whitespace_after_equal=original_node.body[
                                        0
    ...
    def visit_ImportAlias(self, node: cst.ImportAlias):
        self.imported_names.extend(
            [
                n.value
                for n in match.findall(node.name, match.Name(value=match.DoNotCare()))
            ]
        )
    ...
    def __get_required_imports(self):
        ...
        all_req_mods = find_required_modules(
            self.all_applied_types, self.imported_names
        )
        all_type_names = set(
            chain.from_iterable(
                map(lambda t: regex.findall(r"\w+", t[0]), self.all_applied_types)
...
class TypeApplierExtendedBranch(cst.CSTTransformer):
    """
    It applies (inferred) type annotations to a source code file.
    Specifically, it applies the type of arguments, return types, and variables' type.
    """
